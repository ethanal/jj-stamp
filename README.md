...
- **Squash a file:** use the **down-arrow-into-node** button in its header, or hover/focus its sidebar row for the squash button. This queues every remaining changed line in that file, independent of the current line selection. Read-only files and unavailable destinations stay disabled.
